...
namespace PLCCommunication_v2.Panasonic
{
    public class PLCReceivingPacket : IPLCReceivingPacket
    {
        ...
        public PLCReceivingPacket(byte[] receivedData, EPLCDeviceCode code, string address)
        {
            ...
            int result1;
            if (int.TryParse(address, out result1))
            {
            ...
            else
            {
                int result2;
                int result3;
                if (int.TryParse(address.Substring(0, address.Length - 1), out result2) && result2 < 1000 && int.TryParse(address.Substring(address.Length - 1, 1), NumberStyles.HexNumber, (IFormatProvider)null, out result3))
                {
                    string str = string.Empty;
                    if (address.Length < 4)
                        str = "0000" + address;
                    this.ContactAddress = str.Substring(str.Length - 4, 4);
                    this.Address = result2 % 1000 * 10 + result3;
                }
                else
                {
